...
# usage: bench/run_all.sh <outdir> <label> <bench args...>   (VECLIB_MAXIMUM_THREADS=default leaves it unset)
...
vt=${VECLIB_MAXIMUM_THREADS:-1}
{
  ...
  if [ "$vt" = default ]; then env -u VECLIB_MAXIMUM_THREADS ./build/bench "$@"
  else VECLIB_MAXIMUM_THREADS=$vt ./build/bench "$@"; fi
